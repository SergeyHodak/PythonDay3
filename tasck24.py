# <h2>СRM - система "Разработчики Украины"</h2>
# <summary>Теория</summary>
# <p>Пора сложить вместе все классы, методы и функции, которые мы с вами написали и запустить Web-приложение
# <strong>Разработчики Украины</strong>.</p>
# <p>Для этого мы создали:</p>
# <ul>
# <li><code>IncorrectInput</code> и <code>FieldDecodeError</code> для обработки некорректных данных, введенных пользователем.</li>
# <li>Функцию <code>index_error_decorator</code> для обработки ситуации, когда пользователь вводит неправильный индекс</li>
# <li>Функцию <code>get_rate_stat</code> для расчета зарплатной статистики.</li>
# <li>Класс <code>DataField</code>, который является родительским для всех полей нашей CRM и отвечает за преобразование
# объектов <code>DataField</code> в словарь, логику создания новых объектов, проверку на соответствие значения
# записанного в объекте и аргумента.</li>
# <li>Классы <code>RateField</code>, <code>PhoneField</code>, <code>SkillField</code>, <code>CityField</code>, <code>FirstNameField</code> для полей нашей CRM, которые отвечают за корректные названия
# полей и валидацию введенных данных.</li>
# <li>Функцию <code>field_decoder</code>, которая превращает JSON объекты в <code>DataField</code> объекты.</li>
# <li>Класс <code>Developer</code>, который отвечает за объединение нескольких полей <code>DataField</code> в один объект —
# запись в нашей CRM, а также за поиск по всем или некоторым полям.</li>
# <li>Класс <code>AddressBook</code>, ответственный за сохранение записей и манипуляции с ними, поиск по всем записям,
# сохранение данных в JSON и восстановаления JSON, а также за объект этого класса <code>AB</code>, с которым будет работать
# Web-приложение.</li>
# <li>Объект <code>app</code> — Web-приложение</li>
# <li>Функции, отвечающие за логику взаимодействия Web-приложения и <code>AddressBook</code></li>
# </ul>
# <p>Остается добавить код, который запускает наше Web-приложение, чтобы мы могли им пользоваться.</p>
# <p>Для запуска тестового сервера Flask (тестовый сервер не требует установки дополнительных пакетов)
# в объекте <code>app</code> используется  метод <code>run</code>, который может принимать некоторое количество аргументов/настроек.</p>
# <p>Нас прежде всего интересует адрес сервера для доступа к Web-приложению. Адрес состоит из двух
# частей: <code>host</code> и <code>port</code>, и имеет вид: <code>http://host:port</code>. По умолчанию сервер запускается
# таким образом, чтобы быть доступным только с компьютера на котором он запущен по адресу:
# <code>127.0.0.1:5000</code>. Чтобы открыть доступ к серверу всем компьютерам локальной сети, следует указать
# адрес <code>0.0.0.0</code>. Порт можно оставить без изменений, или указать другой. Браузер по умолчанию ожидает,
# что Web-приложение будет отвечать на порту <code>80</code>, но для запуска сервера на <code>80</code> порту необходимо
# предоставить серверу права администратора, что не рекомендуется. Обычно разработчики используют порты
# <code>8080</code> или <code>8888</code>.</p>
...
# </div>
# <p>Модифицируйте функцию <code>main</code>, ответственную за запуск Web-приложения так, чтобы сервер был доступен
# всем компьютерам в локальной сети на порту <code>8080</code>.</p>
...
def main():
    app.run(host="0.0.0.0", port=8080)  # эту строчку нужно было дописать
